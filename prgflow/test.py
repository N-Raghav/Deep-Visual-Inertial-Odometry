...
from prgflow.dataset import VIOPairsDataset
from prgflow.evaluate import run_sequence
...
from prgflow.preprocess import list_trajectories, process_one
...
def parse_args():
    ...
    p.add_argument("--checkpoint", type=str, required=True,
                   help="Path to model checkpoint (e.g. checkpoints/vio/best.pt).")
    ...
    p.add_argument("--results_dir", type=str, default="results/vio_test",
    ...
    p.add_argument("--skip_patch_eval", action="store_true",
                   help="Skip the patch-level (l2/e_trans/e_scale/acc) evaluation.")
...
def patch_eval_trajectory(model, traj, pairs_root, device, batch_size, num_workers, crop_size):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            per_trans = torch.linalg.norm(pred[:, 1:] - label[:, 1:], dim=-1)
...
def main():
    ...
    if not args.skip_patch_eval:
        ...
        header = (
            f"{'sequence':<22} {'n':>6} {'l2':>9} {'eT_mean':>10} {'eT_med':>10} "
            f"{'eT_p95':>10} {'eS_mean':>10} {'eS_med':>10} {'acc%':>8}"
        )
        ...
        for traj in trajectories:
            ...
            if m is None:
                ...
                continue
            log_print(
                f"{traj:<22} {m['n_pairs']:>6d} {m['l2_mean']:>9.4f} "
                f"{m['e_trans_mean_px']:>10.3f} {m['e_trans_median_px']:>10.3f} "
                f"{m['e_trans_p95_px']:>10.3f} {m['e_scale_mean_px']:>10.3f} "
                f"{m['e_scale_median_px']:>10.3f} {m['acc_pct']:>8.2f}",
                log_file,
            )

        log_print("-" * len(header), log_file)
        for key in ["l2_mean", "e_trans_mean_px", "e_scale_mean_px", "acc_pct"]:
    ...
    if not args.skip_trajectory_eval:
        ...
        for traj in trajectories:
            ...
            try:
                from prgflow.evaluate import plot_sequence
    ...
    summary = {
        ...
        "patch_aggregate": {
            ...
            for k in ["l2_mean", "e_trans_mean_px", "e_scale_mean_px", "acc_pct"]
