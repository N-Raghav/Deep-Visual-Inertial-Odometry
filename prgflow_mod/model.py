...
from prgflow_mod.squeezenet import SHead, THead, model_size_mb
from prgflow_mod.warp import compose, decompose_to_pseudosim, pseudo_similarity_matrix, warp_image
...
class PRGFlow(nn.Module):
    def __init__(self, patch_size=128, base_channels=32, fire_blocks=4, dropout=0.7):
        ...
        self.y_head_1 = SHead(base_channels=base_channels, fire_blocks=fire_blocks, dropout=dropout)
        self.y_head_2 = SHead(base_channels=base_channels, fire_blocks=fire_blocks, dropout=dropout)

    def _delta_to_matrix(self, delta, kind, H, W):
        B = delta.shape[0]
        zeros = torch.zeros(B, device=delta.device, dtype=delta.dtype)
        if kind == "t":
            return pseudo_similarity_matrix(zeros, delta[:, 0], delta[:, 1], H, W, yaw=zeros)
        elif kind == "y":
            return pseudo_similarity_matrix(zeros, zeros, zeros, H, W, yaw=delta[:, 0])
        return pseudo_similarity_matrix(delta[:, 0], zeros, zeros, H, W, yaw=zeros)
    ...
    def forward(self, patch_t, patch_t1, return_matrix=False):
        ...
        s, yaw, tx, ty = decompose_to_pseudosim(H_total, H, W, extract_yaw=True)
        pred = torch.stack([s, yaw, tx, ty], dim=1)
        if return_matrix:
            return pred, H_total
        return pred
