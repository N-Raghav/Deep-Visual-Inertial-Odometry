...
from prgflow_mod.attitude import Madgwick
from prgflow_mod.warp import warp_image
...
class VIOFusion:
    ...
    def reset(self, q0=None):
        ...
        self.yaw_est = self._yaw_from_rotation(self.madgwick.R())
    ...
    @staticmethod
    def _yaw_from_rotation(R):
        return float(np.arctan2(R[1, 0], R[0, 0]))

    def _tilt_from_madgwick(self):
        R_imu = self.madgwick.R()
        imu_yaw = self._yaw_from_rotation(R_imu)
        return self._yaw_rotation(-imu_yaw) @ R_imu

    def _rotation_with_predicted_yaw(self):
        return self._yaw_rotation(self.yaw_est) @ self._tilt_from_madgwick()
    ...
    def step(self, frame, imu_samples_since_last_frame, altitude, dt):
        for gyro, accel, dt_imu in imu_samples_since_last_frame:
            gyro_tilt = np.asarray(gyro, dtype=np.float64).copy()
            gyro_tilt[2] = 0.0
            self.madgwick.step(gyro_tilt, accel, dt_imu)
        R_now = self._rotation_with_predicted_yaw()
        ...
        s, yaw, tx, ty = float(pred[0]), float(pred[1]), float(pred[2]), float(pred[3])
        ...
        self.yaw_est = self._wrap_angle(self.yaw_est + yaw)
        R_now = self._rotation_with_predicted_yaw()

        self.prev_frame = frame_tensor
        self.prev_patch = frame_patch
        self.prev_R = R_now
        return self.position.copy(), R_now.copy(), np.array([s, yaw, tx, ty], dtype=np.float64)
