...
def pseudo_similarity_matrix(s, tx, ty, H, W, yaw=None):
    device = None
    dtype = None
    for x in (s, tx, ty, yaw) if yaw is not None else (s, tx, ty):
        if torch.is_tensor(x):
            device = x.device
            dtype = x.dtype
            break
    ...
    if yaw is not None:
        yaw = _batchify_param(yaw, batch, device, dtype)
    else:
        yaw = torch.zeros(batch, device=device, dtype=dtype)
    ...
    out[:, 1, 1] = a * cos_yaw
    out[:, 2, 2] = 1.0
    out[:, 0, 2] = tx + (1.0 - a) * cx * cos_yaw + (1.0 - a) * cy * (-sin_yaw)
    out[:, 1, 2] = ty + (1.0 - a) * cx * sin_yaw + (1.0 - a) * cy * cos_yaw
    return out
...
def decompose_to_pseudosim(H_mat, H=None, W=None, extract_yaw=True):
    H_mat = _batchify_homography(H_mat)

    if extract_yaw:
        a = torch.sqrt(H_mat[:, 0, 0] ** 2 + H_mat[:, 1, 0] ** 2)
        yaw = torch.atan2(H_mat[:, 1, 0], H_mat[:, 0, 0])
        s = a - 1.0
        tx = H_mat[:, 0, 2]
        ty = H_mat[:, 1, 2]
        ...
        if H is not None and W is not None:
            ...
            cos_yaw = torch.cos(yaw)
            sin_yaw = torch.sin(yaw)
            tx = tx - (1.0 - a) * cx * cos_yaw - (1.0 - a) * cy * (-sin_yaw)
            ty = ty - (1.0 - a) * cx * sin_yaw - (1.0 - a) * cy * cos_yaw
        return s, yaw, tx, ty
    else:
...
if __name__ == "__main__":
    ...
    yaw0 = torch.tensor([0.1, -0.05], dtype=torch.float32)
    tx0 = torch.tensor([2.0, -3.5], dtype=torch.float32)
    ty0 = torch.tensor([-1.0, 4.0], dtype=torch.float32)
    H0 = pseudo_similarity_matrix(s0, tx0, ty0, H_img, W_img, yaw=yaw0)
    s1, yaw1, tx1, ty1 = decompose_to_pseudosim(H0, H_img, W_img, extract_yaw=True)
    err = max(
        (s0 - s1).abs().max().item(),
        (yaw0 - yaw1).abs().max().item(),
        (tx0 - tx1).abs().max().item(),
        (ty0 - ty1).abs().max().item(),
    )
    print("param round-trip (with yaw):", err)
    ...
    print("warp/unwarp (with yaw):", (img_mid - rec_mid).abs().mean().item())
