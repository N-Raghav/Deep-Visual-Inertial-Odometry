...
from prgflow_mod.dataset import VIOPairsDataset, split_trajectories
from prgflow_mod.loss import (
    accuracy_percent,
    composite_loss,
    rotation_loss_geodesic,
    scale_error_px,
    translation_error_px,
    translation_loss_mse,
    yaw_error_rad,
)
...
def parse_args():
    ...
    p.add_argument("--checkpoint_dir", type=str, default="checkpoints/vio_imp")
    p.add_argument("--log_dir", type=str, default="logs/vio_imp")
...
def run_epoch(model, loader, optimizer, device):
    ...
    for batch in loader:
        ...
        total_rot_loss += float(rotation_loss_geodesic(pred, label).detach()) * bs
        total_trans_loss += float(translation_loss_mse(pred, label).detach()) * bs
        ...
        total_yaw += float(yaw_error_rad(pred, label).detach()) * bs
        total_acc += float(accuracy_percent(pred, label, patch_size=patch_t.shape[-1]).detach()) * bs
        n += bs

    if n == 0:
        return {"loss": 0.0, "l_rot": 0.0, "l_trans": 0.0, "e_trans": 0.0, "e_scale": 0.0, "e_yaw": 0.0, "acc": 0.0}

    return {
        "loss": total_loss / n,
        "l_rot": total_rot_loss / n,
        "l_trans": total_trans_loss / n,
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(
            ...
            f"train loss={train_metrics['loss']:.4f} lrot={train_metrics['l_rot']:.4f} ltrans={train_metrics['l_trans']:.4f} "
            f"et={train_metrics['e_trans']:.3f}px es={train_metrics['e_scale']:.3f}px ey={train_metrics['e_yaw']:.4f}rad acc={train_metrics['acc']:.2f}% | "
            f"val loss={val_metrics['loss']:.4f} lrot={val_metrics['l_rot']:.4f} ltrans={val_metrics['l_trans']:.4f} "
            f"et={val_metrics['e_trans']:.3f}px es={val_metrics['e_scale']:.3f}px ey={val_metrics['e_yaw']:.4f}rad acc={val_metrics['acc']:.2f}%"
        )
    ...
    print(
        ...
        f"lrot={test_metrics['l_rot']:.4f} "
        f"ltrans={test_metrics['l_trans']:.4f} "
        f"et={test_metrics['e_trans']:.3f}px "
        f"es={test_metrics['e_scale']:.3f}px "
        f"ey={test_metrics['e_yaw']:.4f}rad "
    ...
    writer.add_hparams(
        ...
        },
        {
            ...
            "hparam/test_e_yaw": test_metrics["e_yaw"],
