...
def process_one(traj_dir, out_dir, crop_size, pad, overwrite):
    ...
    for i in range(len(frame_rows) - 1):
        ...
        R_rel = R_t.T @ R_t1
        euler = Rotation.from_matrix(R_rel).as_euler('xyz')
        yaw = float(euler[2])

        label = np.array([s, yaw, tx, ty], dtype=np.float32)
...
def process_one_job(args):
    traj_dir, out_dir, crop_size, pad, overwrite = args
    return traj_dir.name, process_one(traj_dir, out_dir, crop_size, pad, overwrite)
...
def main():
    ...
    jobs = [
        (traj_dir, out_root / traj_dir.name, args.crop_size, args.pad, args.overwrite)
    ...
    print(f"preprocessing {len(jobs)} trajectories with {num_workers} workers")
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(process_one_job, job) for job in jobs]
