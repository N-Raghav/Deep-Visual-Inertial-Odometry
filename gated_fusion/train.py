"""
Train the gated tight-fusion network.

Strongly recommended workflow:

    1. Train ``vision_only`` and ``imu_only`` to convergence.
    2. ``--vision_checkpoint``, ``--airio_checkpoint`` from those runs.
    3. ``--freeze_backbones`` for the first few epochs (gate + heads
       only); then a brief joint fine-tune with a smaller lr.

Loss is ``smooth_l1`` on the relative translation plus a geodesic
SO(3) loss on the rotation (same as ``vision_only/loss.py``).
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train the gated fusion network.")
    ...
    p.add_argument("--warmup_epochs", type=int, default=5,
                   help="Number of initial epochs with frozen backbones.")
    ...
    p.add_argument("--checkpoint_dir", type=str, default="checkpoints/gated_fusion")
    p.add_argument("--log_dir", type=str, default="logs/gated_fusion")
...
def run_epoch(
    ...
    model.train(train)
    sums = {"total": 0.0, "trans": 0.0, "rot_rad": 0.0, "gate_mean": 0.0}
    n = 0
    for batch in loader:
        ...
        with torch.cuda.amp.autocast(enabled=scaler is not None):
            ...
            total, trans_loss, rot_loss = pose_loss(
        ...
        sums["trans"] += float(trans_loss.detach())
        sums["rot_rad"] += float(rot_loss.detach())
        sums["gate_mean"] += float(out["gate"].detach().mean())
        n += 1
    ...
    return {
        ...
        "gate_mean": sums["gate_mean"] / n,
    }


def _set_backbone_freeze(model: GatedFusionNet, frozen: bool) -> None:
...
def _build_optimizer(model: GatedFusionNet, lr: float) -> torch.optim.Optimizer:
...
def main() -> None:
    ...
    print(f"GatedFusion train seqs ({len(train_seqs)}): {train_seqs}")
    print(f"GatedFusion val   seqs ({len(val_seqs)}): {val_seqs}")
    ...
    for epoch in range(args.epochs):
        # Switch to joint fine-tune after warm-up.
        if epoch == args.warmup_epochs and args.warmup_epochs > 0:
            print(f"[gated_fusion] unfreezing backbones at epoch {epoch + 1}; "
        ...
        for k in ("total", "trans", "rot_deg", "gate_mean"):
            writer.add_scalar(f"train/{k}", tr[k], epoch)
            writer.add_scalar(f"val/{k}", va[k], epoch)
        writer.add_scalar("lr", lr, epoch)

        print(
            f"[gated_fusion epoch {epoch + 1:03d}/{args.epochs}] "
            f"train total={tr['total']:.4f} trans={tr['trans']:.4f} "
            f"rot={tr['rot_deg']:.3f}deg gate={tr['gate_mean']:.3f} | "
            f"val total={va['total']:.4f} trans={va['trans']:.4f} "
            f"rot={va['rot_deg']:.3f}deg gate={va['gate_mean']:.3f} | "
            f"lr={lr:.2e}"
        )
